...
export default function CspmSettingsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    setError("");
    try {
      const created = await apiPost<CloudAccount>("/api/v1/cloud-accounts", {
  ...
  return <div className="content settings-content">
    ...
    <CollapsibleForm hasData={accounts.length > 0} title="Cadastrar conta cloud" summary={<small className="muted">{accounts.length} cadastrada{accounts.length === 1 ? "" : "s"}</small>}>
