...
Se for usar a UI "Launch an App from GitHub" do Fly (em vez do `fly deploy --config` via CLI), ela cria a config do zero a partir do Dockerfile escolhido e não lê os `fly.toml` deste repo - reveja manualmente: **Internal port** (`3000` pro `durtone-api`, nenhum pros workers, `8080` pro `durtone-edge`) e a lista completa de secrets acima (a UI só sugere as chaves que existem em `.env.example`, não as sensíveis). `EDGE_FLEET_TOKEN` (no `durtone-api`) e `DURTWALL_FLEET_TOKEN` (no `durtone-edge`) são nomes diferentes pro **mesmo valor** - não é uma variável "faltando", é o mesmo segredo com nome diferente em cada app.