...
Não é "uma aplicação" - são **4 apps Fly separados**, cada um com seu próprio `fly.toml` e conjunto de secrets. `apps/api`, `apps/durtguardian` e `apps/durtscope` dependem de `packages/*` via workspace, então cada um tem seu próprio `Dockerfile.<app>` na raiz (`Dockerfile.api`, `Dockerfile.durtguardian`, `Dockerfile.durtscope`) que builda com a **raiz do repo como contexto** (não a pasta do app) - são quase idênticos entre si, só o `WORKDIR` final muda; ficam separados (em vez de um só parametrizado por `--build-arg`) porque a UI "Launch an App from GitHub" do Fly não dá pra passar build args. `apps/durtwall` é um módulo Go independente, com `Dockerfile`/`fly.toml` próprios dentro da própria pasta (contexto = a própria pasta, não a raiz).
...
1. **Criar o app uma vez, via CLI** (usa o `--dockerfile` que a UI não tem): `flyctl launch --dockerfile Dockerfile.api --config fly.api.toml --copy-config --name durtone-api --org <sua-org> --region gru --no-deploy` (troque pros outros dois trocando o nome/config/dockerfile). Isso exige `flyctl auth login` na sua conta - não é algo que eu deva fazer por você.
2. **Deploys seguintes, via GitHub Actions** (`.github/workflows/fly-deploy-{api,durtguardian,durtscope,durtwall}.yml`, já criados neste repo) - cada um roda `flyctl deploy --config fly.<app>.toml` (que respeita o `dockerfile` do dentro do `fly.toml`, sem precisar detectar nada) quando os arquivos daquele app mudam na branch `main`. Só falta criar o secret **`FLY_API_TOKEN`** nas configurações do repositório no GitHub (Settings → Secrets and variables → Actions) com um token de deploy (`flyctl tokens create deploy -a durtone-api`, um por app ou um só de org) - sem isso os workflows falham no login do flyctl.

`durtone-edge` (DurtWall) é diferente: o Dockerfile dele já se chama `Dockerfile` de verdade, dentro de `apps/durtwall/` - se a UI do GitHub deixar você apontar o "root directory" pra essa pasta, a detecção funciona normalmente pra esse.

Reveja manualmente ao criar cada app: **Internal port** (`3000` pro `durtone-api`, nenhum pros workers, `8080` pro `durtone-edge`) e a lista completa de secrets acima. `EDGE_FLEET_TOKEN` (no `durtone-api`) e `DURTWALL_FLEET_TOKEN` (no `durtone-edge`) são nomes diferentes pro **mesmo valor** - não é uma variável "faltando", é o mesmo segredo com nome diferente em cada app.